...
local function cleanupContext(contextId)
  if activeContextIds[contextId] then
    if Config.Menus == "lation_ui" then
      pcall(function()
        exports.lation_ui:removeMenu(contextId)
      end)
    else
      pcall(function()
        lib.unregisterContext(contextId)
      end)
    end
    activeContextIds[contextId] = nil
  end
end
...
local function openShop(mechanicId, shopIndex)
  ...
  local contextId = "shop_" .. mechanicId .. "_" .. shopIndex
  ...
  activeContextIds[contextId] = true
  
  pcall(function()
    if Config.Menus == "lation_ui" then 
      exports.lation_ui:registerMenu(context) 
      exports.lation_ui:showMenu(contextId)
    else 
      ...
      if shop.usePed then
        ...
        local zoneData = {
          ...
          onExit = function()
            ...
            -- Cleanup any active shop context when leaving the zone
            local contextId = "shop_" .. mechanicId .. "_" .. shopIndex
            cleanupContext(contextId)
          end,
          inside = function()
...
AddEventHandler("onResourceStop", function(resource)
  ...
  for contextId in pairs(activeContextIds) do
    cleanupContext(contextId)
  end
  activeContextIds = {}
