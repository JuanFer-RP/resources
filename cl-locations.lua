...
function createMechanicZonesAndBlips()
  ...
  for _, zone in ipairs(zones) do zone:remove() end  
  ...
  for _, mechanicData in pairs(mechanicsData) do
    local name, label = mechanicData.name, mechanicData.label 
    local mechanicConfig = Config.MechanicLocations[name] or {} 
